...
class Turtle:
    ...
    def __init__(self, command_delay=50):
        ...
        turtle_instances[self._uid] = self
    ...
    def speed(self, s):
        ...
        # mappe linear auf delay-Bereich [100..2]
        self._step_size = s
        max_d, min_d = 100, 2
        span = max_d - min_d
    ...
    def showcode(self, mode='play'):
        ...
        # 1) In[]-History holen (letzte Zelle)
        try:
            shell = get_ipython()
        ...
        # 2) HTML-escape und split in Zeilen
        raw = raw.replace('\r\n', '\n').replace('\r', '\n')
        escaped = html.escape(raw)
        # 3) JS-Aufruf: Code block + Controls initialisieren
        ...
        if hasattr(self, 'command_delay'):
            self._orig_delay = self.command_delay
            self.command_delay = max(self._orig_delay, 200)
        self._code_mode = True # Flag für Code-Modus
        
        bridge_js = f"""
        ;(function() {{
        // robustes Kernel-Handle
        var kernel = (window.Jupyter && Jupyter.notebook && Jupyter.notebook.kernel)
                    || (window.Jupyter && Jupyter.notebook && Jupyter.notebook.kernel)
                    || (window.require ? require('base/js/namespace').notebook.kernel : null);
        if (!kernel) {{
            console.warn('Turtle-Bridge: kein Kernel gefunden');
            return;
        }}
        ['Stop','Step','Play'].forEach(function(label) {{
            var btn = document.getElementById('turtle_btn_' + label + '_{self._uid}');
            if (!btn) return;
            // entferne alte Handler
            btn.replaceWith(btn.cloneNode(true));
            btn = document.getElementById('turtle_btn_' + label + '_{self._uid}');
            // nun neu binden
            btn.addEventListener('click', function() {{
            var method = label === 'Stop' ? 'pause'
                        : label === 'Play' ? 'resume'
                        : 'step';
            var cmd = "turtle_instances['{self._uid}']." + method + "()";
            kernel.execute(cmd);
            }});
        }});
        }})();
        """
        display(Javascript(bridge_js))
